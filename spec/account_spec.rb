...
describe Account do
  ...
  it 'returns detailed transaction message' do
    ...
    expected_output= { transactioncode: 'WRXP45P', balance: new_balance , amount: 200 , time: @date ,  status: true }
    expect(subject.withdraw(200, 7, 1234)).to eq expected_output
  end
  it 'returns failure message for excess amount' do
    expected_output= {balance: subject.balance , time: @date , status: false}
  ...
  it 'deducts transaction fees' do
    new_balance = subject.balance - (200)
    expected_output= { transactioncode: 'WRXP45P', balance: new_balance , amount: 200 , time: @date ,status: true }
    expect(subject.withdraw(200, 98, 1234)).to eq expected_output
  end
end
